...
{{- $saNamespace := index . "namespace" | default $envAll.Release.Namespace }}
{{- $releaseName := index . "releaseName" | default $envAll.Release.Name }}
{{- $clusterBinding := index . "clusterBinding" }}
{{- $defaultRoleName := printf "%s-%s-%s" $releaseName $saNamespace $saName }}
{{- $roleName := index . "roleName" | default $defaultRoleName }}

{{- $rbacOpts := dict "serviceAccountName" $saName "namespace" $saNamespace "releaseName" $releaseName "clusterBinding" $clusterBinding "roleName" $roleName }}
